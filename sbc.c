...
static int sbc_unpack_frame_header(const uint8_t *data, struct sbc_frame *frame,
                            size_t len) {
    if (len < 4)
        return -1;
		...
		struct sbc_frame *frame, size_t len)
{
	...
	if (rc = sbc_unpack_frame_header(data, frame, len) < 0) 
		return rc;
...
static int mmsbc_unpack_frame(const uint8_t *data, struct sbc_frame *frame,
                             size_t len) {
    int rc;
    if (len < 4)
        return -1;
...
static void sbc_set_defaults(sbc_t *sbc, unsigned long flags)
{
	...
	if (priv->msbc) {
            if (priv->mmsbc) { //BR
                ...
                priv->unpack_frame = mmsbc_unpack_frame;
			} else {
                priv->pack_frame = msbc_pack_frame;
